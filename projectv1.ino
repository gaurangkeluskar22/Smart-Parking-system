...
// buzzer pin 
#define buzzerPin 10
String apiKey = "0HI1IC0K5MLX68HT";
...
//    // if car present and slot is not occupied
//    if(Slot>0){
//      Firebase.setInt(fbdo,"slot4 status",1);
//      slot4status=1;
//      Slot=Slot-1;
//      occ4=true;
//      display_available_slot_message(Slot);
...
//    }
//    else{
//      if(Slot<4 and occ4==true and digitalRead(IR4) != LOW){
//        Firebase.setInt(fbdo,"slot4 status",0);
//        slot4status=0;
//        occ4=false;
//        Slot = Slot+1;
//        display_available_slot_message(Slot);
...
void send_h_t_data_to_firebase(float h,float t){
  ...
  Firebase.pushJSON(fbdo, "DHT11", json);
    ...
    postStr += "&field6=";
    postStr += String(slot3status);
    postStr += "&field7=";
    postStr += String(slot4status);
